...
if ~skip_smp_training,
    ...
    save('Data/Dictionary/smp_patches_x5_p3.mat', 'Xh', 'Xl');
    skip_dictionary_training = false;
end;

if ~skip_dictionary_training,
    load('Data/Dictionary/smp_patches_x5_p3.mat');
    [Dh, Dl] = coupled_dic_train(Xh, Xl, codebook_size, lambda);
    save('Data/Dictionary/Dictionary_x5_p3.mat', 'Dh', 'Dl');
else
    load('Data/Dictionary/Dictionary_x5_p3.mat');
end;

%% =====================================================================
% Process the test image 
fileName ='1.bmp'; %fileName = 'tt3.bmp';%'204.jpg';%'baboon.png'; %'1.bmp';%
...
title('Our method');
imwrite(uint8(ReconIm),sprintf('output/%s_SR_z2.bmp',fileName))
%%
psnrNNIm = psnr(nnIm,testIm);
psnrBIm = psnr(interpIm,testIm);
psnrSRIm = psnr(ReconIm,testIm);
sprintf('PSNR     NN=%f, Bicubic=%f, SR=%f', psnrNNIm, psnrBIm,psnrSRIm)
toc
save('Data/Resluts_x2','psnr*')