...
for zooming =3:5%:4
    ...
    for mm = 1:numel(mahaks)
        ...
        tableDataFileName =  sprintf('%s/Summary_Tbl_%s_x%d.txt',outputDir,mahaks{mm},zooming);
        ...
        fprintf(fid,'}\n');
        fprintf(fid,'\\caption{\\textbf{%s} for the proposed method $(\\nu=0.2)$ and other methods (magnified by a factor of \\textbf{%d}). Each cell demonstrates the average value of the corresponding criteria for the mentioned dataset and method (row, collumn). Cells highlighted by dark and light gray, show the 1st and 2nd winners, respectively.}\n',mahakName,zooming);
        fprintf(fid,'\\label{tab:Summary:%s:X%d}\\\\\n',mahakName,zooming);
        fprintf(fid,'\\hline\n');
